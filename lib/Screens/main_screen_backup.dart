...
class _ProfileScreenState extends State<ProfileScreen> with TickerProviderStateMixin {
        ...
        background: Container(
          ...
          child: Stack(
            children: [
              ...
              // Центральная иконка с подпрыгиванием
              Center(
                child: ScaleTransition(
                  scale: _bounceAnimation,
                  child: Icon(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (authProvider.userBio != null && authProvider.userBio!.isNotEmpty) ...[
            const SizedBox(height: 12),
            Text(
            ...
            children: [
              Expanded(
                child: _buildStatItem(
                  context,
                  'Постов',
                  '${authProvider.userStats?['posts_count'] ?? 0}',
                  Icons.post_add,
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: _buildStatItem(
                  context,
                  'Лайков',
                  '${authProvider.userStats?['likes_count'] ?? 0}',
                  Icons.favorite,
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: _buildStatItem(
                  context,
                  'Подписчиков',
                  '${authProvider.userStats?['followers_count'] ?? 0}',
                  Icons.people,
                ),
              ),
            ],
      ...
      child: Column(
        children: [
          ...
          _buildActionTile(
            ...
            color: Colors.purple,
            onTap: () {
              _showPrivacyDialog(context, authProvider);
            },
          ),
          _buildActionTile(
            icon: Icons.help,
  ...
  void _showChangeAvatarDialog(BuildContext context, AuthProvider authProvider) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Функция изменения аватара будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showNotificationsDialog(BuildContext context, AuthProvider authProvider) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Настройки уведомлений'),
        content: const Text('Функция настроек уведомлений будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showSecurityDialog(BuildContext context, AuthProvider authProvider) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Функция настроек безопасности будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showPrivacyDialog(BuildContext context, AuthProvider authProvider) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Настройки приватности'),
        content: const Text('Функция настроек приватности будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showHelpDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Функция помощи будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showSettingsDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Настройки'),
        content: const Text('Функция настроек будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showLoginDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Функция входа будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
  ...
  void _showRegisterDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Функция регистрации будет доступна в следующем обновлении.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Закрыть'),
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text('Цена: ${(product['price'] ?? 0.0).toStringAsFixed(2)} ₽'),
              const SizedBox(height: 8),
              Text('Остаток: ${product['stock_quantity'] ?? 0}'),
              const SizedBox(height: 8),
              Text('Минимальный запас: ${product['min_stock_level'] ?? 0}'),
              const SizedBox(height: 8),
              Text('Избранный: ${product['is_featured'] == true ? 'Да' : 'Нет'}'),
            ],
          ),
        ),
      ),
...
void _showEditProductDialog(BuildContext context, Map<String, dynamic> product) {
  ...
  final stockController = TextEditingController(text: (product['stock_quantity'] ?? 0).toString());
          ...
          onPressed: () async {
            if (nameController.text.isNotEmpty && 
              ...
              if (success) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Ошибка при обновлении товара')),
                );
              }
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
          ...
          onPressed: () async {
            if (contentController.text.isNotEmpty) {
              ...
              if (success) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Ошибка при создании поста')),
                );
              }
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<AnalyticsProvider>(
      builder: (context, analyticsProvider, child) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('PoSPro Аналитика'),
            backgroundColor: Theme.of(context).primaryColor,
            foregroundColor: Colors.white,
            actions: [
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  const PopupMenuItem(value: 'overview', child: Text('Обзор')),
                  const PopupMenuItem(value: 'sales', child: Text('Продажи')),
                  const PopupMenuItem(value: 'customers', child: Text('Клиенты')),
                  const PopupMenuItem(value: 'predictions', child: Text('Предсказания')),
                  const PopupMenuItem(value: 'ml_models', child: Text('ML Модели')),
                ],
              ),
            ],
          ),
          body: _buildAnalyticsContent(analyticsProvider),
        );
      },
    );
  }

  Widget _buildAnalyticsContent(AnalyticsProvider analyticsProvider) {
    switch (_selectedTab) {
      case 'overview':
        return _buildOverviewTab(analyticsProvider);
      case 'sales':
        return _buildSalesTab(analyticsProvider);
      case 'customers':
        return _buildCustomersTab(analyticsProvider);
      case 'predictions':
        return _buildPredictionsTab(analyticsProvider);
      case 'ml_models':
        return _buildMLModelsTab(analyticsProvider);
      default:
        return _buildOverviewTab(analyticsProvider);
    }
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: _buildMetricCard(
                  'Общая выручка',
                  '${(analyticsProvider.salesAnalytics['total_revenue'] ?? 0.0).toStringAsFixed(0)} ₽',
                  Icons.attach_money,
                  Colors.green,
                ),
              ),
              ...
              Expanded(
                child: _buildMetricCard(
                  'Заказы',
                  '${analyticsProvider.salesAnalytics['total_orders'] ?? 0}',
                  Icons.shopping_cart,
                  Colors.blue,
                ),
              ),
          ...
          Row(
            children: [
              Expanded(
                child: _buildMetricCard(
                  'Клиенты',
                  '${analyticsProvider.customerAnalytics['total_customers'] ?? 0}',
                  Icons.people,
                  Colors.orange,
                ),
              ),
              ...
              Expanded(
                child: _buildMetricCard(
                  'Средний чек',
                  '${(analyticsProvider.salesAnalytics['average_order_value'] ?? 0.0).toStringAsFixed(0)} ₽',
                  Icons.receipt,
                  Colors.purple,
                ),
              ),
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.kpis.map((kpi) => _buildKPICard(kpi)),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.salesTrends.map((trend) => _buildTrendCard(trend)),
          const SizedBox(height: 24),

          // Аномалии
          if (analyticsProvider.anomalies.isNotEmpty) ...[
            Text(
              'Обнаруженные аномалии',
              style: Theme.of(context).textTheme.headlineSmall,
            ),
            const SizedBox(height: 16),
            ...analyticsProvider.anomalies.map((anomaly) => _buildAnomalyCard(anomaly)),
          ],
        ],
      ),
    );
  ...
  Widget _buildSalesTab(AnalyticsProvider analyticsProvider) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.salesForecasts.map((forecast) => _buildForecastCard(forecast)),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          _buildBusinessMetricsGrid(analyticsProvider.businessMetrics),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.benchmarks.map((benchmark) => _buildBenchmarkCard(benchmark)),
        ],
      ),
    );
  }

  Widget _buildCustomersTab(AnalyticsProvider analyticsProvider) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.customerSegments.map((segment) => _buildSegmentCard(segment)),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.customerLifetimeValue.map((clv) => _buildCLVCard(clv)),
        ],
      ),
    );
  }

  Widget _buildPredictionsTab(AnalyticsProvider analyticsProvider) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.predictions.map((prediction) => _buildPredictionCard(prediction)),
        ],
      ),
    );
  }

  Widget _buildMLModelsTab(AnalyticsProvider analyticsProvider) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 16),
          ...analyticsProvider.mlModels.entries.map((entry) => _buildMLModelCard(entry.key, entry.value)),
        ],
      ),
    );
  }
  ...
  Widget _buildKPICard(Map<String, dynamic> kpi) {
    ...
    if (kpi['status'] == 'needs_attention') statusColor = Colors.orange;
    if (kpi['status'] == 'below_target') statusColor = Colors.red;
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(kpi['name'] ?? ''),
        subtitle: Text('Цель: ${kpi['target_value']}'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(
              '${(kpi['current_value'] * 100).toStringAsFixed(1)}%',
              style: TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: statusColor,
  ...
  Widget _buildTrendCard(Map<String, dynamic> trend) {
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          backgroundColor: _getTrendColor(trend['trend']),
          child: Icon(_getTrendIcon(trend['trend']), color: Colors.white),
        ),
        title: Text(trend['metric'] ?? ''),
        subtitle: Text('Точность: ${(trend['forecast_accuracy'] * 100).toStringAsFixed(1)}%'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
  ...
  Widget _buildAnomalyCard(Map<String, dynamic> anomaly) {
    ...
    if (anomaly['severity'] == 'medium') severityColor = Colors.orange;
    if (anomaly['severity'] == 'high') severityColor = Colors.red;
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('Тип: ${anomaly['type']}'),
        trailing: IconButton(
          icon: Icon(Icons.info, color: severityColor),
  ...
  Widget _buildForecastCard(Map<String, dynamic> forecast) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(forecast['period'] ?? ''),
        subtitle: Text('Доверие: ${(forecast['confidence_interval'] * 100).toStringAsFixed(1)}%'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(
              '${(forecast['predicted_revenue'] / 1000000).toStringAsFixed(1)}M ₽',
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: Colors.blue,
            ...
            Text(
              '+${(forecast['growth_rate'] * 100).toStringAsFixed(1)}%',
              style: TextStyle(
                fontSize: 12,
                color: Colors.green,
              ),
  ...
  Widget _buildBusinessMetricsGrid(Map<String, dynamic> metrics) {
    return GridView.count(
      ...
      mainAxisSpacing: 16,
      children: [
        _buildMetricCard('Маржа', '${(metrics['gross_margin'] * 100).toStringAsFixed(1)}%', Icons.pie_chart, Colors.green),
        _buildMetricCard('ROI', '${(metrics['return_on_investment'] * 100).toStringAsFixed(1)}%', Icons.trending_up, Colors.blue),
        _buildMetricCard('Оборачиваемость', '${metrics['inventory_turnover']?.toStringAsFixed(1)}', Icons.rotate_right, Colors.orange),
        _buildMetricCard('DSO', '${metrics['days_sales_outstanding']?.toStringAsFixed(1)} дн.', Icons.schedule, Colors.purple),
      ],
    );
  }
  ...
  Widget _buildBenchmarkCard(Map<String, dynamic> benchmark) {
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          backgroundColor: _getPercentileColor(benchmark['percentile']),
          child: Text(
            '${benchmark['percentile']}',
            style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
          ),
        ),
        title: Text(benchmark['metric'] ?? ''),
        subtitle: Text('Наш результат: ${(benchmark['our_performance'] * 100).toStringAsFixed(1)}%'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(
              'Отрасль: ${(benchmark['industry_average'] * 100).toStringAsFixed(1)}%',
              style: TextStyle(fontSize: 12),
            ),
            Text(
              'Топ: ${(benchmark['top_performers'] * 100).toStringAsFixed(1)}%',
  ...
  Widget _buildSegmentCard(Map<String, dynamic> segment) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('${segment['count']} клиентов'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(
              '${segment['average_order_value']?.toStringAsFixed(0)} ₽',
              style: TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.bold,
                color: Colors.blue,
            ...
            Text(
              'Удержание: ${(segment['retention_rate'] * 100).toStringAsFixed(0)}%',
              style: TextStyle(fontSize: 12),
            ),
          ],
        ),
  ...
  Widget _buildCLVCard(Map<String, dynamic> clv) {
    return Card(
      margin: const EdgeInsets.only(bottom: 12),
      child: ListTile(
        leading: CircleAvatar(
        ...
        title: Text('Клиент ${clv['customer_id']}'),
        subtitle: Text('${clv['total_orders']} заказов'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(
              '${clv['total_spent']?.toStringAsFixed(0)} ₽',
              style: TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.bold,
                color: Colors.green,
            ...
            Text(
              'Прогноз: ${clv['predicted_lifetime_value']?.toStringAsFixed(0)} ₽',
              style: TextStyle(fontSize: 12),
            ),
          ],
        ),
  ...
  Widget _buildPredictionCard(Map<String, dynamic> prediction) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(prediction['type'] ?? ''),
        subtitle: Text('Уверенность: ${(prediction['confidence_level'] * 100).toStringAsFixed(1)}%'),
        trailing: IconButton(
  ...
  Widget _buildMLModelCard(String modelName, Map<String, dynamic> model) {
    return Card(
      margin: const EdgeInsets.only(bottom: 12),
      child: ListTile(
        leading: CircleAvatar(
        ...
        subtitle: Text('${model['model_type']}'),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
  ...
  void _showAnomalyDetails(Map<String, dynamic> anomaly) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Детали аномалии'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Описание: ${anomaly['description']}'),
            const SizedBox(height: 8),
            Text('Тип: ${anomaly['type']}'),
            Text('Серьезность: ${anomaly['severity']}'),
            const SizedBox(height: 8),
            Text('Возможные причины:'),
            ...(anomaly['possible_causes'] as List?)?.map((cause) => Text('• $cause')) ?? [],
            const SizedBox(height: 8),
            Text('Рекомендуемые действия:'),
            ...(anomaly['recommended_actions'] as List?)?.map((action) => Text('• $action')) ?? [],
          ],
        ),
        actions: [
          TextButton(
  ...
  void _showPredictionDetails(Map<String, dynamic> prediction) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Детали предсказания'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Тип: ${prediction['type']}'),
            Text('Уверенность: ${(prediction['confidence_level'] * 100).toStringAsFixed(1)}%'),
            const SizedBox(height: 8),
            if (prediction['type'] == 'demand_forecast') ...[
              Text('Категория: ${prediction['product_category']}'),
              Text('Предсказанный спрос: ${prediction['predicted_demand']}'),
              Text('Следующий месяц: ${prediction['next_month_prediction']}'),
            ],
            if (prediction['type'] == 'customer_churn') ...[
              Text('Риск: ${prediction['risk_level']}'),
              Text('Затронутые клиенты: ${prediction['affected_customers']}'),
              Text('Прогнозируемые потери: ${prediction['predicted_loss']?.toStringAsFixed(0)} ₽'),
            ],
            if (prediction['type'] == 'price_optimization') ...[
              Text('Текущая цена: ${prediction['current_price']?.toStringAsFixed(0)} ₽'),
              Text('Оптимальная цена: ${prediction['optimal_price']?.toStringAsFixed(0)} ₽'),
              Text('Ожидаемый эффект: ${prediction['predicted_impact']}'),
            ],
            const SizedBox(height: 8),
            Text('Рекомендации:'),
            ...(prediction['recommendations'] as List?)?.map((rec) => Text('• $rec')) ?? [],
          ],
        ),
        actions: [
          TextButton(
